#!/bin/bash
LEVEL="complevels/FOSACrazyMonkeys.lvl"
if [ $# == '1' ]
then
    if [[ "$1" = "debug" ]]
    then
	...
	java -jar server.jar -l $LEVEL -c 'java -classpath bin;lib\* dk.dtu.ai.blueducks.BlueDucksClient -Djava.util.logging.SimpleFormatter.format="%1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS %4$s %2$s %5$s%6$s%n" '
fi
