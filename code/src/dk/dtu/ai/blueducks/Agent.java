...
import dk.dtu.ai.blueducks.heuristics.MoveBoxHeuristicFactory;
...
public class Agent {
	...
	private List<State> computePlanStates(Goal goal, State agentState) {
		...
		if (goal instanceof GoToBoxGoal) {
			...
			MoveBoxHeuristicFactory mhf = new MoveBoxHeuristicFactory();
			path = AStarSearch.<State, MoveBoxGoal> getBestPath(agentState, mbGoal, mhf.getHeuristic());
